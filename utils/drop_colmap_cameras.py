...
def drop_cameras_from_colmap(
    ...
    assert truncation or interval

    if interval is not None:
        assert interval >= 1, "--interval must be >= 1"
        assert 0 <= offset < interval, "--offset must be in [0, interval-1]"
    ...
    if interval is not None:
        ...
        if truncation == "front":
            drop_ids = ordered[:-2]
        elif truncation == "back":
            drop_ids = ordered[2:]
        elif truncation == "mid":
            drop_ids = ordered[1:-1]
        elif truncation == "front_back":
            img_len = len(ordered)
            mid_point = img_len//2
            drop_ids = ordered[:mid_point] + ordered[mid_point+2:]
        else:
            raise ValueError("Invalid Truncation", truncation)
    ...
    if interval is not None:
        ...
        postfix = "truncation_" + truncation
